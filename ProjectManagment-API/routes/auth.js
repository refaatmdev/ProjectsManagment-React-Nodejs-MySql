...
const { generateToken, protect } = require("../controllers/jwt");
...
router.post(
  ...
  asyncHandler(async (req, res, next) => {
    ...
    try {
      ...
      if (!email || !password) res.send("email and password is required");
      ...
      if (result) {
        ...
        res.cookie("token", token, {
          path: "/",
          httpOnly: true,
          expires: new Date(Date.now() + 4 * 3600 * 1000), //5 hours
          sameSite: "none",
          secure: true,
        });
        const { id, email, role } = result;
        console.log("now", new Date());
        console.log("addfive", new Date(Date.now() + 60 * 60 * 3));
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    try {
      const result = await isUserRegistered(req.user.email);
      if (result) {
        const { id, email, role } = result;
      ...
      logger.error(
        `${currentTime} - get user information Failed - ${error.message} `
      );
      return res.json({
        message: `get user information Failed - ${error.message}`,
        status: 500,
      });
    }
  })
